  ...
  <div class="container mx-auto p-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            ...
            class="p-2 border rounded w-full"
          />
          <p v-if="state.errors.dateOfBirth" class="text-red-500 text-sm mt-1">{{ state.errors.dateOfBirth }}</p>
        </div>
        ...
        <div>
          ...
          />
          <p v-if="state.errors.mobileNumber" class="text-red-500 text-sm mt-1">{{ state.errors.mobileNumber }}</p>
        </div>
    ...
    <div class="overflow-x-auto bg-white shadow-md rounded-lg">
      <div class="flex mb-4 p-4">
        <input v-model="state.search" placeholder="Search by Name" class="p-2 border rounded w-full mr-2" />
      ...
      <table class="w-full text-left">
        ...
        <tbody>
          <tr v-for="user in paginatedUsers" :key="user.id" class="border-b">
            <td class="p-3">
              <input v-if="user.isEditing" v-model="user.first_name" class="p-2 border rounded w-full" />
              <span v-else>{{ user.first_name }}</span>
            </td>
            <td class="p-3">
              <input v-if="user.isEditing" v-model="user.last_name" class="p-2 border rounded w-full" />
              <span v-else>{{ user.last_name }}</span>
            </td>
            <td class="p-3">
              <input v-if="user.isEditing" type="date" v-model="user.date_of_birth" class="p-2 border rounded w-full" />
              <span v-else>{{ formatDate(user.date_of_birth) }}</span>
            </td>
            <td class="p-3">
              <input v-if="user.isEditing" v-model="user.mobile_number" class="p-2 border rounded w-full" />
              <span v-else>{{ user.mobile_number }}</span>
            </td>
            <td class="p-3">
              <textarea v-if="user.isEditing" v-model="user.address" class="p-2 border rounded w-full"></textarea>
...
export default {
  setup() {
    const state = reactive({
      users: [],
      form: { firstName: '', lastName: '', dateOfBirth: '', mobileNumber: '', address: '' },
      errors: {},
      search: '',
      sortColumn: '',
      currentPage: 1
    ...
    const filteredUsers = computed(() => state.users.filter(u => u.first_name.includes(state.search) || u.last_name.includes(state.search)));

    const sortedUsers = computed(() => {
      const sorted = [...filteredUsers.value];
      if (state.sortColumn === 'first_name') sorted.sort((a, b) => a.first_name.localeCompare(b.first_name));
      if (state.sortColumn === 'date_of_birth') sorted.sort((a, b) => new Date(a.date_of_birth) - new Date(b.date_of_birth));
      return sorted;
    });
